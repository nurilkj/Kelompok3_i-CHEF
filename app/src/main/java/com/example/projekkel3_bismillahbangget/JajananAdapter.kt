...
class JajananAdapter (val arrayList: ArrayList<KatalogMenu>, val context: Context) :
    RecyclerView.Adapter<JajananAdapter.ViewHolder>() {

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItems(katalog: KatalogMenu) {
            itemView.findViewById<ImageView>(R.id.imageJajanan).setImageResource(katalog.image)
            itemView.findViewById<TextView>(R.id.titleJajanan).text = katalog.title
            itemView.findViewById<TextView>(R.id.descJajanan).text = katalog.desc
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.jajanan, parent, false)
        return ViewHolder(v)
    }
