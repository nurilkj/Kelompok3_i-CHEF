...
class ArabicAdapter(val arrayList: ArrayList<KatalogMenu>, val context: Context) :
    RecyclerView.Adapter<ArabicAdapter.ViewHolder>() {

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItems(katalog: KatalogMenu) {
            itemView.findViewById<ImageView>(R.id.imageArabic).setImageResource(katalog.image)
            itemView.findViewById<TextView>(R.id.titleArabic).text = katalog.title
            itemView.findViewById<TextView>(R.id.descArabic).text = katalog.desc
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.arabic, parent, false)
        return ViewHolder(v)
    }
