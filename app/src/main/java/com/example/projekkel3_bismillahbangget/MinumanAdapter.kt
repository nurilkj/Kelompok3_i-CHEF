...
class MinumanAdapter(val arrayList: ArrayList<KatalogMenu>, val context: Context) :
    RecyclerView.Adapter<MinumanAdapter.ViewHolder>() {

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItems(katalog: KatalogMenu) {
            itemView.findViewById<ImageView>(R.id.imageMinuman).setImageResource(katalog.image)
            itemView.findViewById<TextView>(R.id.titleMinuman).text = katalog.title
            itemView.findViewById<TextView>(R.id.descMinuman).text = katalog.desc
        }
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.minuman, parent, false)
        return ViewHolder(v)
    }
