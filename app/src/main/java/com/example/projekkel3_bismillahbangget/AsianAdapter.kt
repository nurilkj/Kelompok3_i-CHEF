...
class AsianAdapter (val arrayList: ArrayList<KatalogMenu>, val context: Context) :
    RecyclerView.Adapter<AsianAdapter.ViewHolder>() {

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItems(katalog: KatalogMenu) {
            itemView.findViewById<ImageView>(R.id.imageAsian).setImageResource(katalog.image)
            itemView.findViewById<TextView>(R.id.titleAsian).text = katalog.title
            itemView.findViewById<TextView>(R.id.descAsian).text = katalog.desc
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.asian, parent, false)
        return ViewHolder(v)
    }
