...
class KatalogAdapter (val arrayList: ArrayList<KatalogMenu>, val context: Context) :
    RecyclerView.Adapter<KatalogAdapter.ViewHolder>() {

    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindItems(katalog: KatalogMenu) {
            itemView.findViewById<ImageView>(R.id.imageNusantara).setImageResource(katalog.image)
            itemView.findViewById<TextView>(R.id.titleNusantara).text = katalog.title
            itemView.findViewById<TextView>(R.id.descNusantara).text = katalog.desc
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.nusantara, parent, false)
        return ViewHolder(v)
    }
